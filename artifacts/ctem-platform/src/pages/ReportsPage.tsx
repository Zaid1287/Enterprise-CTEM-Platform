...
function generateReportPdf(report: any) {
  const lines: string[] = [
    ...
    ``,
    `Report sections included based on type:`,
  ];

  if (report.type === "executive") {
    lines.push(`  • Risk Score Overview`, `  • Critical & High Findings Summary`, `  • Remediation Priorities`, `  • Trend Analysis`);
  } else if (report.type === "technical") {
    lines.push(`  • Full Vulnerability Listing (CVE details)`, `  • CVSS & EPSS Scores`, `  • Port & Service Exposure`, `  • Secrets & Misconfigurations`);
  } else if (report.type === "compliance") {
    lines.push(`  • Compliance Control Status`, `  • Gap Analysis`, `  • Evidence Mapping`, `  • Remediation Roadmap`);
  } else if (report.type === "asset_inventory") {
    lines.push(`  • Full Asset List`, `  • Asset Risk Levels`, `  • Last Scan Dates`, `  • Coverage Map`);
  }

  lines.push(``, `─────────────────────────────────────────────────────────────────────`, ``, `Generated by CTEM Platform  ·  Confidential`);
  ...
  a.href = url;
  a.download = `${report.title.replace(/\s+/g, "_")}_${report.type}.txt`;
  a.click();
