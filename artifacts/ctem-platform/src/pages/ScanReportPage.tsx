...
function downloadScanReportPdf(scan: any, assetReports: any[]) {
  ...
  const lines: string[] = [
    `CTEM Platform — Scan Report`,
    `${"═".repeat(64)}`,
    ``,
    `Report Name:  ${scan?.name ?? `Scan #${scan?.id}`}`,
    `Completed:    ${ts}`,
    `Assets:       ${assetReports.length}`,
    `Total Findings: ${assetReports.reduce((a: number, r: any) => a + (r.summary?.vulnerabilities ?? 0), 0)}`,
    ``,
    `${"═".repeat(64)}`,
  ];
  ...
  for (const asset of assetReports) {
    ...
    if (cves.length > 0) {
      lines.push(``, `  CVEs Found (top 10):`);
      cves.slice(0, 10).forEach((c: any) => {
        lines.push(`    • ${c.cveId ?? "—"}  [${c.severity?.toUpperCase() ?? "?"}]  CVSS: ${c.cvss ?? "—"}  ${c.title ?? ""}`);
      });
    }
  }

  lines.push(``, `${"═".repeat(64)}`, `Generated by CTEM Platform  ·  Confidential`);
