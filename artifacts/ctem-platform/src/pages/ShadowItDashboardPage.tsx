...
import {
  ...
  ChevronRight, Bell, Activity, Wifi, Package,
  Zap, Lock, Radio, Monitor, Cpu, HardDrive, Printer,
  BarChart3, Plus, ExternalLink, Loader2, TrendingUp,
} from "lucide-react";
import {
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="rounded-xl bg-card border border-border/50 overflow-hidden">
        ...
        <div className="px-6 py-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Unsanctioned apps · OAuth risk exposure · Unmanaged network devices
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
          ...
          {/* Exposure Score Band */}
          {data && (
            <div className="mt-5 flex items-center gap-6 bg-background/60 border border-border/50 rounded-xl px-5 py-3 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-medium">{riskLabel}</p>
                </div>
              </div>
        ...
        {[
          {
            ...
            sub2: `${data?.shadowAssets.high ?? 0} high risk`,
          ...
          <Link key={c.label} href={c.href}>
            <div className={cn(
              "group relative overflow-hidden rounded-xl border bg-card bg-gradient-to-br cursor-pointer transition-all duration-200",
      ...
      {/* ── Top Risky OAuth Apps — Card Grid ───────────────────────────────── */}
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        {(!data?.oauthApps.topRisky || data.oauthApps.topRisky.length === 0) ? (
          ...
          <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3">
            {data.oauthApps.topRisky.map((app, idx) => (
              <div
                key={app.id}
                className={cn(
                  "group relative rounded-xl border bg-background/50 p-4 flex flex-col gap-3 hover:bg-accent/30 transition-all hover:shadow-sm cursor-default",
                  app.riskLevel === "critical" ? "border-red-500/20" :
                  app.riskLevel === "high"     ? "border-orange-500/20" :
                  app.riskLevel === "medium"   ? "border-yellow-500/20" :
                  "border-border",
                )}
              >
                {/* Rank + sanction badge */}
                <div className="flex items-center justify-between">
                  ...
                  {!app.isSanctioned && (
                    <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-orange-500/10 text-orange-400 border border-orange-500/20">
                      Unsanctioned
                    </span>
                  )}
                </div>
                ...
                {/* App avatar + name */}
                <div className="flex items-center gap-3">
                  <div className="h-9 w-9 rounded-lg bg-gradient-to-br from-accent to-muted flex items-center justify-center text-sm font-bold shrink-0 border border-border">
                    {app.displayName.slice(0, 2).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold truncate">{app.displayName}</p>
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className={cn("text-[10px] px-2 py-0.5 rounded border font-semibold capitalize", RISK_BG[app.riskLevel])}>
                      {app.riskLevel}
                    </span>
                    <span className="text-[10px] text-muted-foreground font-mono tabular-nums">{app.riskScore}%</span>
                  </div>
                  <div className="h-1 rounded-full bg-accent overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${app.riskScore}%`, background: RISK_COLORS[app.riskLevel] ?? "#888" }}
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  <span>user{app.userCount !== 1 ? "s" : ""}</span>
                </div>
              </div>
            ))}
          </div>
