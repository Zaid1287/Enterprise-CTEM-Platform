...
export default function RunScanDialog({ open, onOpenChange, pipelineTools, assets, onRunComplete, preSelectedAssetIds }: Props) {
  ...
  function selectAllAssets(all: boolean) {
    setAssetConfigs(prev => prev.map(c => {
      if (!all) return { ...c, included: false };
      const asset = assets.find(a => a.id === c.assetId);
      const isVerified = asset?.verificationStatus === "verified";
      return { ...c, included: isVerified ? true : c.included };
    }));
  }
            ...
            <div>
              ...
              <div className="border border-border rounded-lg divide-y divide-border/50 max-h-72 overflow-y-auto">
                {assets.length === 0 ? (
                  ...
                  return (
                    <div key={asset.id} className={cn("transition-colors", isDisabled ? "opacity-50" : cfg.included ? "bg-primary/5" : "")}>
                      <div className="flex items-center gap-3 px-3 py-2.5">
                        <button
                          ...
                          className={cn("w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-colors",
                            isDisabled ? "border-muted-foreground/40 cursor-not-allowed" :
                            cfg.included ? "bg-primary border-primary" : "border-muted-foreground hover:border-foreground")}
                        >
                          {cfg.included && !isDisabled && <Check className="w-2.5 h-2.5 text-white" />}
                        </button>
                        ...
                        {isDisabled && (
                          <span className="text-[10px] text-amber-400 border border-amber-500/30 rounded px-1.5 py-0.5 shrink-0">Unverified</span>
                        )}

                        <span className="text-[10px] bg-accent/60 text-muted-foreground rounded px-1.5 py-0.5 shrink-0">{asset.type}</span>

                        {cfg.included && !isClient && !isDisabled && (
                      ...
                      {cfg.included && cfg.expanded && enabledTools.length > 0 && !isClient && !isDisabled && (
