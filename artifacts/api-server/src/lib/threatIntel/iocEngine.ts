 ...
 * deduplicates via upsert on (type, value, source), and calculates
 * a 0-100 threat score based on source reliability × confidence ×
 * severity × freshness.
 */
...
export function calculateThreatScore(ioc: NormalizedIoc): number {
  ...
  const fresh = freshnessScore(new Date());
  return Math.min(100, Math.round(rel * conf * sev * fresh * 100));
}
  ...
  for (let i = 0; i < iocs.length; i += BATCH_SIZE) {
    ...
    try {
      const result = await db.insert(tiIocsTable)
        .values(rows)
        .onConflictDoUpdate({
          target: [tiIocsTable.type, tiIocsTable.value, tiIocsTable.source],
          set: {
      ...
      // Can't easily distinguish inserts from updates without CTEs, so count all as "added"
      added += result.length;
    } catch (err) {
