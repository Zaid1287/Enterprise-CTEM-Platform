...
const PLATFORM_KEYS: PlatformKeyDef[] = [
  ...
  { key: "gitlab_api_key",         label: "GitLab Personal Access Token", description: "GitLab personal access token (read_api scope) — queries GitLab Search API for public projects and code containing target domain/company names. Without this key, unauthenticated search is used with lower rate limits.", category: "osint" },
  ...
  { key: "google_search_api_key", label: "Google Custom Search API Key", description: "Google Programmable Search Engine API key — used for keyword, social handle, and logo OSINT dorking queries across the web. Get one at console.cloud.google.com.", category: "brand_threat" },
  { key: "google_search_cx",      label: "Google Custom Search Engine ID", description: "Google Programmable Search Engine ID (cx parameter) — paired with the Google Search API key to scope OSINT dork searches. Create your search engine at programmablesearchengine.google.com.", category: "brand_threat" },
  ...
  { key: "ti_alienvault_key",        label: "AlienVault OTX API Key",  description: "AlienVault OTX (Open Threat Exchange) API key — enables pulse subscriptions, IOC feeds, threat actor reports, and malware family data ingestion. Register free at otx.alienvault.com.", category: "threat_intel" },
  { key: "ti_abuseipdb_key",         label: "AbuseIPDB API Key",       description: "AbuseIPDB API key — enriches IOCs with IP abuse reports, confidence scores, and geographic attribution. Higher plan = more lookups/day.", category: "threat_intel" },
