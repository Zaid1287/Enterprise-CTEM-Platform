...
router.get("/graph/attack-surface", requireAuth, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  // Client sees ONLY their assigned assets (cross-tenant)
  if (role === "client") {
    const emptyGraph = { nodes: [], relationships: [], meta: { nodeCount: 0, relationshipCount: 0, assetCount: 0, findingCount: 0, cveCount: 0 } };
    ...
    if (assignedAssets.length === 0) { res.json(emptyGraph); return; }

    const assignedIds = assignedAssets.map(a => a.id);
    const assignedTenantIds = [...new Set(assignedAssets.map(a => a.tenantId))];
    ...
    for (const t of tenants) {
      nodes.push({ id: `org-${t.id}`, labels: ["Organization"], properties: { name: t.name ?? "Organization", tenantId: t.id, plan: t.plan ?? "starter" } });
    }
    ...
    for (const asset of assets) {
      ...
      nodes.push({ id: nodeId, labels: [capitalize(asset.type), "Asset"], properties: { name: asset.name, value: asset.value, type: asset.type, riskLevel: asset.riskLevel, ipAddress: asset.ipAddress, port: asset.port, businessImpact: asset.businessImpact, verificationStatus: asset.verificationStatus, lastScannedAt: asset.lastScannedAt } });
      ...
      if ((asset.type === "subdomain" || asset.type === "url")) {
        for (const parent of assets) {
          if (parent.id !== asset.id && parent.type === "domain" && asset.value.toLowerCase().endsWith(`.${parent.value.toLowerCase()}`)) {
      ...
      if (asset.ipAddress && asset.type !== "ip") {
        ...
        if (ipNodeId && ipNodeId !== nodeId) relationships.push({ id: `r-resolves-${asset.id}`, type: "RESOLVES_TO", startNodeId: nodeId, endNodeId: ipNodeId });
      }
      if (asset.type === "ip" && asset.port) {
        const portId = `port-${asset.id}-${asset.port}`;
        nodes.push({ id: portId, labels: ["Port"], properties: { port: asset.port, protocol: "tcp", assetId: asset.id } });
    ...
    for (const f of findings) {
      ...
      nodes.push({ id: findingId, labels: ["Vulnerability", "Finding"], properties: { title: f.title, severity: f.severity, status: f.status, cve: f.cve, cvss: f.cvss, epss: f.epss, isKev: f.isKev, riskScore: f.riskScore } });
      ...
      if (f.cve) {
        ...
        if (!seenCves.has(cveKey)) { seenCves.add(cveKey); nodes.push({ id: cveId, labels: ["CVE"], properties: { id: cveKey, url: `https://nvd.nist.gov/vuln/detail/${cveKey}` } }); }
        relationships.push({ id: `r-cve-${f.id}`, type: "REFERENCES_CVE", startNodeId: findingId, endNodeId: cveId });
      }
    }

    res.json({ nodes, relationships, meta: { nodeCount: nodes.length, relationshipCount: relationships.length, assetCount: assets.length, findingCount: findings.length, cveCount: seenCves.size } });
    return;
  }
